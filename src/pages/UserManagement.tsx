...
export default function UserManagement() {
  ...
  const handleGoalRequest = async (userId: string, goalId: string, status: 'approved' | 'rejected') => {
    try {
      await api.patch(`/goals/${goalId}`, { status: status === 'approved' ? 'active' : 'rejected' });
      toast.success(`Goal request ${status} successfully`);

      // Update local state if selected user is open
      if (selectedUser && selectedUser._id === userId) {
        setSelectedUser({
          ...selectedUser,
          learningGoals: selectedUser.learningGoals?.map(g =>
            g._id === goalId ? { ...g, status: status === 'approved' ? 'active' : 'rejected' } : g
          )
        });
      }
    } catch (error) {
                                ...
                                {goal.status === 'pending' && (
                                  <div className="flex items-center gap-2 mt-2">
                                    <Button
                                      ...
                                      onClick={() => handleGoalRequest(selectedUser._id, goal._id, 'approved')}
                                    >
                                    ...
                                    <Button
                                      ...
                                      onClick={() => handleGoalRequest(selectedUser._id, goal._id, 'rejected')}
                                    >
